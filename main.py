...
EPISODES = {
    "Midterm": {"1": "FILE_ID_EP1", "2": "FILE_ID_EP2", "3": "FILE_ID_EP3"},
    "Vikings": {"1": "FILE_ID_VIK1", "2": "FILE_ID_VIK2"},
    "Alice in Borderland": {"1": "FILE_ID_Alice1", "2": "FILE_ID_Alice2"}
}
    ...
    if not check_subscription(user_id):
        ...
        for ch in REQUIRED_CHANNELS:
            keyboard.add(InlineKeyboardButton(
                f"اشترك في {ch}", url=f"https://t.me/{ch[1:]}"
            ))
        keyboard.add(InlineKeyboardButton(
            "✔️ تحقّق من الاشتراك", callback_data="check_sub"
        ))
        bot.send_message(
            message.chat.id,
...
def show_series_menu(chat_id):
    ...
    keyboard.add(InlineKeyboardButton(
        f"👥 عدد المستخدمين: {get_user_count()}", callback_data="show_count"
    ))
...
def show_episodes(chat_id, series_name):
    ...
    bot.send_message(chat_id, f"🎞 اختر حلقة من {series_name}:", reply_markup=keyboard)
...
def callback(call):
    ...
    if data == "show_count":
        bot.send_message(call.message.chat.id, f"👥 عدد المستخدمين الحاليين: {get_user_count()}")
        return
